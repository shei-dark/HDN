...
path="/group/jug/Sheida/pancreatic beta cells/download/high_c1/contrastive/test/"
patch_size = 64
# model = torch.load("/group/jug/Sheida/HDN models/21012024/Contrastive_MAE_best_vae.net")
...
files = glob(path+"mito_golgi_gra/*.tif")
...
for i in files:
    ...
    with torch.no_grad():
        ...
        for idx in range(len(sample['mu'])):
            ...
            imwrite(path+"label/"+str(idx+1)+"/"+i[i.rfind("/"):i.rfind(".")]+".tif", output.reshape(22*(2**(4-idx)), 24*(2**(4-idx))))
