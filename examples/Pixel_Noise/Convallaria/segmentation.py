...
with torch.no_grad():
    ...
    mus = output["mu"]
    # Iterate over the 0th dimension (1400)
    ...
    # Convert the list of numpy arrays to a single numpy ndarray
...
for i in tqdm(range(0, H-patch_size, 1)):
    for j in tqdm(range(0, W-patch_size, 1)):
...
for i in tqdm(range(min_row, max_row+1)):
    for j in tqdm(range(min_col, max_col+1)):
...
dp.save_pickle('label_pred.pkl', label_pred)