...
def objective(trial):
    ...
    scale = trial.suggest_int("scale", 1, 10)
    # scale = 8
    ...
    model_name = "2D_HVAE"
    directory_path = "/group/jug/Sheida/HVAE/2D/scale_optuna/"
    noiseModel = None
...
if __name__ == "__main__":
    # Create a study object and set the optimization direction
    study = optuna.create_study(direction="minimize")
    
    # Start the optimization process, passing the objective function and the number of trials
    study.optimize(objective, n_trials=50)

    # Print out the best trial's results
    print("Best trial:")
