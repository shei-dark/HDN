...
def train_network(model, lr, max_epochs,train_loader, val_loader, 
                  gaussian_noise_std, model_name, directory_path="./",
                  batch_size=8, alpha=1, beta=1, gamma=1,
                  nrows=4, max_grad_norm=None, amp=True, gradient_scale=8192):
    ...
    for epoch in range(max_epochs):
        ...
        for idx, (x, y, z) in tqdm(enumerate(train_loader)):
            ...
            outputs = boilerplate.forward_pass(x, y, device, model, gaussian_noise_std, amp=amp)
        ...
        run.log(
            {   
                ...
                "cl pos pair" : np.mean(running_cl_pos),
                "cl neg pair" : np.mean(running_cl_neg),                      
        ...
        np.save(model_folder+"train_reco_loss.npy", np.array(inpainting_loss_train_history))
        ...
        with torch.no_grad():
            for i, (x, y, z) in enumerate(val_loader):
                ...
                val_loss = alpha * val_inpainting_loss + beta * val_kl_loss + gamma * val_cl_loss
                running_validation_loss.append(val_loss)

        run.log(
            {
                "val total loss": np.mean(running_validation_loss)
            }
        )
        model.train()
